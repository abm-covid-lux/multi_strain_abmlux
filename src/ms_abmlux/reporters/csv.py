...
from ms_abmlux.reporters import Reporter
...
class CumulativeCaseCountsByStrain(Reporter):
    """Reporter that writes to a CSV file as it runs."""
    ...
    def __init__(self, telemetry_bus, config):
        ...
        self.subscribe("strains.list", self.header)
        self.subscribe("cumulative_cases_by_strain.update", self.update_counts)
        self.subscribe("simulation.end", self.stop_sim)

    def header(self, strains_list):
        """Called when the simulation starts.  Writes headers and creates the file handle."""
        ...
        for strain in strains_list:
            header += [strain + " (resident)"]
        self.writer.writerow(header)

    def update_counts(self, clock, cumulative_cases_by_strain):
        """Update the CSV, writing a single row for every clock tick"""

        row = [clock.t, clock.iso8601()]
        row += cumulative_cases_by_strain
        self.writer.writerow(row)
...
class StrainCounts(Reporter):
    ...
    def update_counts(self, clock, strains_counts):
        """Update the CSV, writing a single row for every clock tick"""
...
class UnemployedCounts(Reporter):
    """Reporter that writes to a CSV file as it runs."""
    ...
    def __init__(self, telemetry_bus, config):
        ...
        self.subscribe("simulation.start", self.start_sim)
        self.subscribe("unemployed.update", self.update_unemployed)
        self.subscribe("simulation.end", self.stop_sim)
    ...
    def start_sim(self):
        ...
        # Write header
        header = ["tick", "date", "Agents Unemployed"]
        self.writer.writerow(header)

    def update_unemployed(self, clock, unemployed):
        """Save data and reset daily counts"""

        row = [clock.t, clock.now().date(), unemployed]
        self.writer.writerow(row)
